		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="入睡时间" prop="rushuishijian">
							<el-date-picker
								class="list_date"
								v-model="form.rushuishijian"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								type="datetime"
								:readonly="!isAdd||disabledForm.rushuishijian?true:false"
								placeholder="请选择入睡时间" />
						</el-form-item>
					</el-col>
					<el-col :span="12">
						<el-form-item label="起床时间" prop="qichuangshijian">
							<el-date-picker
								class="list_date"
								v-model="form.qichuangshijian"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								type="datetime"
								:readonly="!isAdd||disabledForm.qichuangshijian?true:false"
								placeholder="请选择起床时间" />
						</el-form-item>
					</el-col>
					<el-col :span="12">
						<el-form-item label="睡眠时长" prop="shuimianshizhang">
							<el-input class="list_inp" v-model="shuimianshizhang" :readonly="true" placeholder="睡眠时长" />
						</el-form-item>
					</el-col>
					<el-col :span="12">
...
<script setup>
	...
	const disabledForm = ref({
		...
		rushuishijian : false,
		qichuangshijian : false,
		shuimianshizhang : false,
		tongjishijian : false,
		userid : false,
	})
	...
	const id = ref(0)
	const type = ref('')
	const shuimianshizhang =computed(()=>{
		let d = form.value
		let a = 'h.rushuishijian-h.qichuangshijian'
		let n = a.split('-')
		let hour = getHour(d[n[0].split('h.')[1]], d[n[1].split('h.')[1]])
		form.value.shuimianshizhang = hour
		return hour
	})
	//methods
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='rushuishijian'){
					form.value.rushuishijian = row[x];
					disabledForm.value.rushuishijian = true;
					continue;
				}
				if(x=='qichuangshijian'){
					form.value.qichuangshijian = row[x];
					disabledForm.value.qichuangshijian = true;
					continue;
				}
				if(x=='shuimianshizhang'){
					form.value.shuimianshizhang = row[x];
					disabledForm.value.shuimianshizhang = true;
					continue;
				}
				if(x=='tongjishijian'){
